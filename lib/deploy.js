...
function sftpDeploy(opts) {
  ...
  ssh.createConnection(function(err, conn) {
    ...
    conn.exec('mkdir -p /tmp/remote-script/; rm -rf /tmp/remote-script/*; tar -x -C /tmp/remote-script', function(err, rstdin) {
      ...
      tessel.tarCode(ret.pushdir, {node: true}, function(err, bundle) {
        ...
        rstdin.once('finish', function() {
          ...
          conn.exec('node /tmp/remote-script/' + ret.relpath, function(err, stream) {
            if (err) throw err;
