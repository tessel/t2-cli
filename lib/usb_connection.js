...
USBConnection.prototype.exec = function(command, callback) {
  ...
  Daemon.openProcess(self, function(err, proc) {
    if (err) {
      ...
      proc.control.write(command);
      // Return the callback with the streams
...
USBConnection.prototype._processArgsForTransport = function(command) {
  ...
  function quoteRemover(match, p1, p2, p3) {
    return p2;
  }

  // For each command
  command.forEach(function(arg, i) {
    // Ensure outer quotes are stripped
    command[i] = arg.replace(/(\'|\")(.*)(\'|\")/, quoteRemover);
