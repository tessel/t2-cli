...
try {
  ...
  haveusb = false;
  logs.warn('No USB controller found on this system.');
}
...
USB.Connection.prototype._receiveMessages = function() {
  ...
  self.epIn.on('error', function(e) {
    ...
    if (self.closed) {
      ...
      logs.warn('Error reading USB message endpoint:', e);
  ...
  return new Promise(function(resolve, reject) {
    ...
    try {
      ...
      if (e.message === 'LIBUSB_ERROR_ACCESS' && process.platform === 'linux') {
        logs.warn('Please run `sudo tessel install-drivers` to fix device permissions.\n(Error: could not open USB device.)');
      }
      // Reject if error
