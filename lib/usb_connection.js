...
var REQ_BOOT = 0xBB;
var USB = {};
var minTimeout = 2000; // Each connection has 2 seconds to receive a response.
    ...
    Daemon.openProcess(self, function(err, proc) {
      if (err) {
        ...
        proc.control.once('finish', function commandWritten() {
          // Clear the command failed timeout
          clearTimeout(controlTimeoutHandle);
          // Resolve with the process
          resolve(proc);
        });

        controlTimeoutHandle = setTimeout(function() {
          // Remove the listener on the process
          proc.control.removeAllListeners();
          // Deliver the bad news
          reject('USB connection to Tessel 2 found but unable to connect... Are you sure T2 is booted?');
        }, minTimeout);
      }
    });
  });
  ...
  return new Promise(function(resolve, reject) {
    ...
    try {
      ...
      if (e.message === 'LIBUSB_ERROR_ACCESS' && process.platform === 'linux') {
        logs.err('Please run `sudo tessel install-drivers` to fix device permissions.\n(Error: could not open USB device.)');
      }
      // Reject if error
