...
USBConnection.prototype._write = function(chunk, enc, callback) {
  ...
  if (this.closed && typeof callback === 'function') {
  ...
  return new Promise(function(resolve, reject) {
    ...
    try {
      ...
      if (e.message === 'LIBUSB_ERROR_BUSY') {
        rejection = 'Device is in use by another process';
      }
      // Reject if error
      return reject(new Error(rejection), self);
    }
    ...
    self.intf.setAltSetting(2, function(error) {
      ...
      if (!self.epIn || !self.epOut) {
        return reject(new Error('Device endpoints could not be loaded'), self);
      }
...
function findConnections() {
  return new Promise(function(resolve) {
      var connections = usb.getDeviceList()
        .map(function(dev) {
          if ((dev.deviceDescriptor.idVendor === TESSEL_VID) &&
            (dev.deviceDescriptor.idProduct === TESSEL_PID)) {
            return new USBConnection(dev);
          }
        })
        // RWQ: This can probably just be `.filter(Boolean);`
        .filter(function(x) {
          return x;
        });
      resolve(connections);
