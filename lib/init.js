...
function generateSample() {
  ...
  // Rust
  if (options.lang.toLowerCase() == 'rust') {
        ...
        fs.exists(file_toml, function(exists) {
          ...
          fs.mkdir(dir_cargo, function(err) {
            ...
            fs.mkdir(dir_src, function(err) {
              ...
              fs.createReadStream(path.resolve(__dirname + './../resources/'+file_config)).pipe(fs.createWriteStream('.cargo/'+file_config));
              fs.createReadStream(path.resolve(__dirname + './../resources/'+file_main)).pipe(fs.createWriteStream(dir_src+file_main));
              fs.createReadStream(path.resolve(__dirname + './../resources/'+file_toml)).pipe(fs.createWriteStream(file_toml));
  ...
  // Python
  else if (options.python.toLowerCase() == 'python') {
