...
function generateSample() {
  ...
  // Rust
  if (options.rust) {
    // Make .cargo directory
    fs.mkdir(
      options.rust+"/.cargo",
      function(err) {
        if (err) { console.error("Could not make .cargo directory"); }
        // copy over config file
        fs.createReadStream(path.resolve(__dirname + './../resources/config')).pipe(fs.createWriteStream(options.rust+"/.cargo/config"));
        // copy over blinky example
        fs.createReadStream(path.resolve(__dirname + './../resources/main.rs')).pipe(fs.createWriteStream(options.rust+'/src/main.rs'));
        // update Cargo.toml to have tessel dependency
        fs.appendFile(
    ...
    console.log('Wrote \"Blinky\" example to '+options.rust+'/src/main.rs');
  }
