        ...
        if (opts.single) {
          ...
          if (opts.push) {
            ...
            prom = prom.then(() => {
              ...
              .then(() => {
                return this.simpleExec(commands.createFolder(Tessel.REMOTE_APP_PATH));
              });
          }
        }
      ...
      return exportables.findProject(opts).then((project) => {
        ...
        return opts.lang.preBundle(opts).then(() => {
          return opts.lang.tarBundle(opts).then((bundle) => {
            ...
            // Log write
            log.info(`Writing project to ${memtype} on ${colors.magenta(tessel.name)} (${(bundle.length / 1000)} kB)...`);
...
exportables.start = function(tessel, entryPoint) {
  return tessel.simpleExec(commands.moveFolder(Tessel.REMOTE_RUN_PATH, Tessel.REMOTE_APP_PATH))
    .then(() => {
