      ...
      .then(function(remoteProcess) {
        ...
        actions.findProject(opts.entryPoint).then(function(project) {
          ...
          actions.tarBundle(resolved).then(function(bundle) {
            ...
            var deployProgress = progress({
              length: bundle.length,
              time: 100
            });

            // Create a new progress bar
            var bar = new ProgressBar('  deploying [:bar] :percent', {
              complete: '=',
              incomplete: ' ',
              width: 20,
              total: bundle.length
            ...
            deployProgress.on('progress', function(progress) {
              bar.tick(progress.transferred);
            });

            // Pipe the progress counte to the stdin of the process
            deployProgress.pipe(remoteProcess.stdin);
            // Write the code bundle to the hardware
            deployProgress.end(bundle);
          });
