  ...
  stopRunningScript(self, filepath, function() {
    ...
    deleteOldCode(self, filepath, function() {
      ...
      createNewCodeFolder(self, filepath, function() {
        ...
        untarStdin(self, filepath, opts, function(ret, entryPoint) {
          // If we are pushing
          if (push) {
            // Push the script into flash
            pushScript(self, entryPoint);
          }
          // If we are pushing
          else {
            // Push the code into ram
            runScript(self, filepath, ret, entryPoint);
          }
        });
      });
    });
...
function untarStdin(t, filepath, opts, callback) {
  t.connection.exec(commands.untarStdin(filepath), function(err, remoteProcess) {
    ...
    // TODO: This section unnecessarily synchronous
    t.getName(function (err, tesselname) {
      ...
      tesselClassic.tarCode(ret.pushdir, {node: true}, function(err, bundle) {
        ...
        // Log write
        logs.info('Writing ' + opts.entryPoint + ' to ' + memtype + ' on ' + tesselname + ' (' + bundle.length/1000 + ' kB)....');
        ...
        remoteProcess.once('close', function() {
          logs.info('Deployed.');
          callback && callback(ret, opts.entryPoint);
        });
...
function runScript(t, filepath, ret, entryPoint) {
  ...
  t.connection.exec(commands.runScript(filepath, ret.relpath), function(err, remoteProcess) {
    ...
    remoteProcess.once('close', function(code, signal) {
      // End the connection
      t.connection.end(function() {
        // Exit the process
        process.exit(1);
...
function pushScript(t, entryPoint) {
  ...
  writeToFile(t, startPushedScript.bind(this, t, entryPoint));
}
