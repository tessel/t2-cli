  ...
  return new Promise(function(resolve) {
    return t.connection.exec(commands.runScript(filepath, entryPoint))
      .then(function(remoteProcess) {
        ...
        function echoSIGINT() {
          // pass the SIGINT to the remote process
          remoteProcess.stdout.signal('SIGINT');
        }

        // Once we get a SIGINT from the console
        process.once('SIGINT', echoSIGINT);

        // When the stream closes
        remoteProcess.once('close', function removeListener() {
          // Remove the process listener if it still exists
          if (process.listeners('SIGINT').indexOf(echoSIGINT) !== -1) {
            process.removeListener('SIGINT', echoSIGINT);
          }

          // Report the stopped script
