...
actions.findProject = function(file, single) {
  return new Promise(function(resolve, reject) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    return t.connection.exec(commands.untarStdin(filepath))
      ...
      .then(function(remoteProcess) {
        ...
        actions.findProject(opts.entryPoint, opts.single).then(function(project) {
          ...
          var bundleStreamPromise;
          if (opts.single) {
            var singleFile = path.resolve(process.cwd(), project.program);
            bundleStreamPromise = Promise.resolve(fs.readFileSync(singleFile));
          } else {
            var resolved = path.resolve(process.cwd(), project.pushdir);
            bundleStreamPromise = new Promise(function(resolve, reject) {
              actions.tarBundle(resolved).then(resolve, reject);
            });
          }

          bundleStreamPromise.then(function(bundle) {
              return reject(error);
            }
