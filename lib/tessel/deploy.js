...
function untarStdin(t, filepath, opts, callback) {
   t.connection.exec(commands.untarStdin(filepath), function(err, remoteProcess) {
    remoteProcess.stderr.on('data', function(d) {
      tessel.logs.err("Unable to deploy code:", d.toString());
  ...
  // Tar up the code to improve transfer rates
    tessel.tarCode(ret.pushdir, {node: true}, function(err, bundle) {
      tessel.logs.info('Bundled. Writing to T2....', bundle.length)
...
function pushScript(t, filepath) {
  // Write the node start file
  writeToFile(t, startPushedScript.bind(this, t));
}

function writeToFile(t, callback) {
  t.connection.exec(commands.openStdinToFile(PUSH_PATH + PUSH_START_SCRIPT_NAME), function(err, remoteProcess) {
    if (err) throw err;
