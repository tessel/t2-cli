...
// Language: *
const PUSH_START_SHELL_SCRIPT = 'start';
...
actions.push = function(tessel, opts) {
  ...
  return actions.writeToFile(tessel, opts.resolvedEntryPoint)
    .then(() => actions.start(tessel, opts.resolvedEntryPoint));
};


actions.writeToFile = function(tessel, entryPoint) {
  return new Promise(function(resolve, reject) {
    // Path of the script to run a Node project
    var shellScriptPath = path.posix.join(Tessel.REMOTE_PUSH_PATH, PUSH_START_SHELL_SCRIPT);
    // Open a stdin pipe tp the file
    tessel.connection.exec(commands.openStdinToFile(shellScriptPath), (err, remoteProcess) => {
      if (err) {
        return reject(err);
      }
      ...
      remoteProcess.once('close', function() {
        ...
        tessel.connection.exec(commands.chmod('+x', shellScriptPath), (err, remoteProcess) => {
          if (err) {
            return reject(err);
          }
