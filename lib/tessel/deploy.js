...
Tessel.prototype.restartScript = function(opts) {
  ...
  return new Promise(function(resolve, reject) {
    return actions.findProject(opts.entryPoint).then(function(project) {
      return self.connection.exec(commands.readFile(filepath + project.entryPoint))
          ...
          remoteProcess.once('close', function() {
            if (error.length) {
              if (error.indexOf('No such file or directory') !== -1) {
                error = '"' + project.entryPoint + '" not found on ' + self.name;
              }
              ...
              if (isPush) {
                ...
                actions.startPushedScript(self, project.entryPoint)
                  .then(resolve).catch(reject);
              } else {
                // Start the script in RAM
                actions.runScript(self, filepath, project.entryPoint)
                  .then(resolve).catch(reject);
              }
            }
          });
