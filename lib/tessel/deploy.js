...
Tessel.prototype.deployScript = function(opts) {
  ...
  function pushBundle(filepath) {
    return function pushBundlePromise() {
      return new Promise(function(resolve) {
        var prom = Promise.resolve();

        if (opts.single) {
          // Always be sure the appropraite dir is created
          prom.then(actions.execRemoteCommand(self, 'createFolder', filepath));
        } else {
          // Delete any code that was previously at this file path
          prom.then(actions.execRemoteCommand(self, 'deleteFolder', filepath)
            // Create the folder again
            .then(actions.execRemoteCommand(self, 'createFolder', filepath)));
        }

        // Bundle and send tarred code to T2
        prom.then(resolve(actions.sendBundle(self, filepath, opts)));
      });
    };
  };
};
