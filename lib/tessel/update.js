...
Tessel.prototype.updateOpenWRT = function(image) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    // Warn that it's best to have a LAN connection
    if (self.connection.connectionType !== 'LAN') {
      logs.warn("It's best to have your T2 connected to WiFi for fast update speeds. USB only can take upwards of 20 minutes...");
    }
...
Tessel.prototype.updateFirmware = function(image) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (!connection) {
      return reject('Must have Tessel connected over USB to complete update. Aborting...');
    }
