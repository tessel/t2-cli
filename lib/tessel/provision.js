...
actions.setDefaultKey = function(keyPath) {
  return new Promise(function(resolve, reject) {
    ...
    if (typeof keyPath !== 'string') {
      return reject('SSH key path must be a string type.');
    }

    if (!fs.statSync(keyPath).isFile() ||
      !fs.statSync(keyPath + '.pub').isFile()) {
      return reject(keyPath + ' does not contain valid public and private SSH keys.');
    }

    Tessel.TESSEL_AUTH_PATH = keyPath;
