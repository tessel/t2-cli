...
Tessel.isProvisioned = function() {
  // TODO: checking for beeing provisioned should happen different than checking for keys
  // a present key file does not ensure the Tessel is provisioned.
  // A Tessel is provisioned if tessel.lanConnection.auth.privateKey !== undefined
  // Because this method is called after the opts.key is setting 
  // filepath it is a double check of an existing key

  // replaces deprecated existsSync
...
actions.setDefaultKey = function(opts) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    try {
      ...
      debug('Keyfiles not found', e);
      reject('Could not find ' + filepath + ' and its public key (.pub)');
    }
  });
};
