...
Tessel.prototype.setName = function(name, callback) {
  ...
  if (typeof name != 'string' || !validHostnameRegex.test(name)) {
    // throw an error
    return callback && callback(new Error("Invalid name: " + name + ". It must be a valid hostname string. See http://en.wikipedia.org/wiki/Hostname#Restrictions_on_valid_host_names."));
  }
  ...
  if (opts.reset) {
    ...
    self._getMACAddress(function(err, addr) {
      ...
      self.setName(defaultName, function(err) {
        ...
        tessel.logs.info('Reset the name of the device to', defaultName);
    ...
    self.getName(function(err, oldName) {
      ...
      self.setName(opts.name, function(err) {
        if (err) return callback && callback(err);

        tessel.logs.info("Changed name of device", oldName, "to", opts.name);
