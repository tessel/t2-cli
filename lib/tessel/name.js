...
var defaultNamePrefix = 'Tessel-';Tessel.prototype.getName = function(callback) {
  var self = this;
...
Tessel.prototype.setName = function(name, callback) {
  ...
  if ((typeof name !== 'string' || !rvalidHostName.test(name)) && typeof callback === 'function') {
    // throw an error
  ...
  if (opts.reset) {
    ...
    self.getName(function(err, oldName) {
      ...
      self.setName(opts.newName, function(err) {
        ...
        tessel.logs.info('Changed name of device', oldName, 'to', opts.newName);
