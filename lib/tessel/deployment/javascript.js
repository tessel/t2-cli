...
exportables.tarBundle = function(opts) {
  ...
  // Convert `lists.includes` into includeRules
  lists.includes.forEach(include => includeRules.push(`node_modules/**/${include}`));
  ...
  var ignoreRules = exportables.glob.rules(target, '.tesselignore').concat(includeNegateRules);
