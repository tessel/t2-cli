    ...
    var requests = binaries.map(details => {
      return new Promise((resolve, reject) => {
        ...
        if (binaryModulesUsed.has(releasename)) {
          ...
          let a = existing.modulePath.match(/\//g).length;
          let b = details.modulePath.match(/\//g).length;
      ...
      project.collect((error, dependencies) => {
        if (error) {
          ...
          dependencies.forEach(dependency => {
            ...
            var isCompressionSafe = !lists.compressionBlacklist[dependency.packageName];
            var isIgnored = false;
