...
Tessel.seekSelected = function(opts){
  return new Promise(function(resolve, reject) {
    ...
    if (opts.timeout && typeof opts.timeout === 'number') {
      ...
      setTimeout(stopSeeker, 3 * 1000);
    }
    console.log("Searching accessable Tessels ...")
    ...
    Tessel.seekSelected(opts)
     ...
     if(tessels.length >= 2){
      ...
      if(tessels[0].connection.authorized){
        ...
        console.log('"t2 key generate" might help :-)');
        process.exit();
      }
      
    } else {
      console.log('Sorry no nearby Tessels found!');
      process.exit();
    }
    function runSSH(id){
         // clear console
        ...
        console.log('Connect to '+tessels[id].connections[0].host+'...');
        ...
        ch.on('exit',function(e,signal){
          if(e === 0){
            // clear console
            process.stdout.write("\u001b[2J\u001b[0;0H");
            console.log('Connection to '+tessels[id].connections[0].host+' closed!\n');
          }
          if(e === 255){
            console.log('Connection to '+tessels[id].connections[0].host+' closed!\n');
          }
      ...
      function makeMenu(tessel, index, array){
        if(tessel.connection.connectionType === 'LAN'){
            ...
            // TODO: Because of no real Tessel 2 present - not implemented now
            if(!tessel.connection.authorized){
