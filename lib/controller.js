  ...
  return new Promise(function(resolve, reject) {
    return updates.requestBuildList().then(function(builds) {
      ...
      if (opts.list) {
        ...
        builds.slice(-10).forEach(function(build) {
          ...
          sha = tessel.then(function(tessel) {
            ...
            return tessel.fetchCurrentBuildInfo()
              ...
              .then(function(currentSHA) {
                return updates.findBuild(builds, 'sha', currentSHA).version;
              });
          });
        }

        // Once we have our Tessel and current version
        return Promise.all([tessel, sha]).spread(function(tessel, currentVersion) {
          ...
          if (opts.version) {
            ...
            var verifiedVersion;
            if (version === 'latest') {
              verifiedVersion = builds[build.length - 1].version;
            }
            // If they requested a specific version
            else {
              if (updates.findBuild(builds, 'version', version)) {
                verifiedVersion = version;
              }
            }

            // Check if the current build is the same or newer if this isn't a forced update
            if (!opts.force && currentVersion >= version) {
              ...
              return controller.closeTesselConnections([tessel])
                .then(reject.bind(this, message), reject.bind(this, message));
            } else {
              if (!opts.force) {
                // If it is a newer version, let's update...
                logs.info('New firmware version found...' + version);
              }

              logs.info('Updating ' + tessel.name + ' to latest version (' + version + ')...');
              // Fetch the requested version
              return controller.updateTesselWithVersion(opts.force, tessel, currentVersion,  build)
                .then(resolve, reject);
            }
