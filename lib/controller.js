var Tessel = require('./tessel/tessel'),
  ...
  runner = require('../test/test_rig/runner');
        ...
        runHeuristics(opts, foundTessels)
          .then(function logSelected(tessel) {
            ...
            if (tessel) {
              ...
              logs.info('Multiple Tessels found, could not set a default. Please specify the Tessel');
            }
            // Finish this function
            resolve(foundTessels);
          });
      }
    ...
    .then(function selectTessel(collector) {
      ...
      for (var i = 0; i < collector.length; i++) {
        ...
        // If this is a LAN Tessel
          ...
          // We have multiple LAN Tessels which is an issue only 
          // if there is muti usb as well, or if there are 0 usbs
          else {
            multiLan = true;
          }
        }
...
function runTests(opts) {
  return getTessel(opts)
    .then(function(selectedTessel) {
      return runner.runTests(opts, selectedTessel);
    })
}

function runWifiTest(opts, foundTessel) {
  function connectAndRun(tessel){
    return tessel.connection.open()
      .then(function(){
        return tessel.connectToNetwork(opts)
        .then(function(){
          return runner.runPingTest(opts, tessel, 10000);
        });
      })
      .catch(function(err){
        console.log("connect and run err", err);
      })
    
  }

  if (foundTessel) {
    return connectAndRun(foundTessel);
  } else {
    return getTessel(opts)
    .then(function(selectedTessel){
      return connectAndRun(selectedTessel);
    });
  }
}

function runUSBTest(opts, foundTessel) {
  return runner.runUSBTest(opts, foundTessel);
}

function runEthernetTest(opts, foundTessel){
  // clear out wifi to make sure only ethernet is connected
  return foundTessel.connectToNetwork({'ssid': 'openwrt', 'password': 'default'})
    .then(function(){
      return runner.runPingTest(opts, foundTessel, 2000);
    })
}

module.exports.runEthernetTest = runEthernetTest;
module.exports.runUSBTest = runUSBTest;
module.exports.runWifiTest = runWifiTest;
