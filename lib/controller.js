    ...
    provision.setDefaultKey(opts).then(function() {
      ...
      seeker.on('tessel', function displayResults(tessel) {
        ...
        if (tessel.connection.connectionType === 'LAN' && !tessel.connection.authorized) {
          if (tessel.lanConnection.auth.privateKey) {
            debug('Tessel is not authorized but provisioned! (--key PRIVATEKEY could help)');
            note = '(Not Authorized)';
          } else {
...
controller.provisionTessel = function(opts) {
  ...
  return new Promise(function(resolve, reject) {
      if (Tessel.isProvisioned()) {
        if (opts.force) {
          cp.exec('rm ' + Tessel.TESSEL_AUTH_KEY + Tessel.TESSEL_AUTH_KEY + '.pub', function(error) {
            if (error) {
              reject(error);
            } else {
              resolve();
