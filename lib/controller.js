...
controller.menu = {
  ...
  prepare: function(opts, tessels, resolve, reject, title, callback) {
    //return new Promise(function(resolve, reject) {
    ...
    // create menu entries
    tessels.forEach(function(tessel, index) {
      controller.menu.make(tessel, index, rtm); 
    });
    ...
    if (!opts.menu) {
      ...
      controller.menu.show(rtm, resolve, reject);
    } else {
  ...
  make: function(tessel, index, rtm) {
  ...
  show: function(rtm, resolve, reject) {
    // Menu navigation
