  ...
  return Promise.all([
          ...
          lan.findConnections()
            ...
            .reduce(function(validConnections, lan_connection){
              ...
              tessel.logs.info('LAN CONNECTION ip: ', lan_connection.ip, ', host: ', lan_connection.auth.host, ', Authorized: ', lan_connection.authorized);
...
function getTessel(opts) {
  ...
  return Promise.settle([usb.findConnections(),lan.findConnections()])
    ...
    .reduce(function(collector, connection){
      ...
      if(process.env.SELECTED_TESSEL && process.env.SELECTED_TESSEL === connection.host ){
        match = connection
      }
      // Over ride with command line options
      // Match name
      if(opts.name && opts[2] === connection.auth.host){
        match = connection;
      }

      // Match ip
      if(opts.ip && opts[2] === connection.ip){
        match = connection;
      }
