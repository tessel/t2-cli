var usb = require('./usb_connection')
  ...
  , _ = require('lodash')
  , commands = require('./tessel/commands');
  ;
  ...
  return Promise.all([
            ...
            .map(function(usb_connection){
              return new Promise(function(resolve, reject) {
                ...
                t.getName(function(err) {
                  ...
                  tessel.logs.info('USB CONNECTION ', 'name: ', t.name);
                   // Pass connections through for consistency
          ...
          lan.findConnections(timeout)
            ...
            .reduce(function(validConnections, lan_connection){
              ...
              return new Promise(function(resolve, reject) {
                var t = new Tessel(lan_connection);
                t.getName(function(err) {
                  if (err) {
                    return reject(err);
                  }
                  // Pass connections through for consistency
                  tessel.logs.info('LAN CONNECTION ip: ', t.connection.ip, ', name: ', t.name, ', Authorized: ', t.connection.authorized);
...
function getTessel(opts) {
  ...
  var timeout = opts.timeout || 1.5;
  return Promise.settle([usb.findConnections(),lan.findConnections(timeout)])
    ...
    .map(function(connection){
      return new Promise(function(resolve, reject) {
        var t = new Tessel(connection);
        if (connection.connectionType == "USB" || connection.authorized) {
          t.getName(function(err, name) {
            if (err) {
              return reject(err);
            }
             // Pass connections through for consistency
    ...
    .reduce(function(collector, device){
      // TODO: Refactor to use UUID once implemented
      // TODO: Put logic in for USB once implemented remove following case
      if(!collector.selected && device.connection.connectionType === 'USB'){
        collector.default = device;
        collector.selected = device;
        return collector;
      }
      ...
      // TODO: Remove slice and use .name once USB is implemented
      if(process.env.SELECTED_TESSEL && process.env.SELECTED_TESSEL === device.name ){
        match = device;
      }

      // Over ride with command line options
      // Match name
      if(opts.name && opts.name === device.name){
        match = device;
      }
      ...
      if(match && collector.selected){
        tessel.log.warn("Multiple matches found. Using tessel at IP address: " + device.connection.ip);
        return collector;
      }
    ...
    .then(function(connections){
      return new Promise(function(resolve, reject) {
        ...
        var device = connections.default;

        // If name option provided
        if(opts.name && connections.selected){
          // Use selected tessel instead
          device = connections.selected;
        }

        // Log connection info
        var message = "Connected over " + device.connection.connectionType + "."
        if (device.connection.connectionType === 'LAN'){
          message += " IP ADDRESS: " + device.connection.ip;
        }
...
module.exports.setupTessel = setupTessel;
