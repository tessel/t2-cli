...
Reboots your tessel
*/
controller.reboot = function(opts) {
  return new Promise(function(resolve, reject) {
    var host;
    if (opts.ip) {
      Tessel.get = function(opts) {
        return new Promise(function(resolve) {
          debug('Rebooting remote IP: ', opts.ip);
          resolve();
        });
      };
    } else if (!opts.name) {
      logs.err('Either --name or --ip is required!');
      resolve();
    }
    Tessel.get(opts)
      .then(function(tessel) {
        if (!tessel && opts.ip) {
          debug('Prevented looking for Tessels!');
          host = opts.ip;
        } else if (tessel && !opts.ip) {
          debug('Tessel found!');
          host = tessel.lanConnection.ip;
        }
        var command = 'ssh root@' + host + ' -i ' + opts.key + ' \'reboot ';
        if (opts.time ||  opts.delay)  {
          command += ' -d ' + opts.time || opts.delay;
        }
        if (opts.force) {
          command += ' -f ';
        }
        ...
        command += '\'';
        debug(command);
        var reboot = cp.exec(command);

        function rebootHandler(e) {
          if (e === 255) {
            logs.warn('Not authorized (maybe wrong key)');
            resolve();
          } else if (e === 127) {
            // command not found
            logs.warn('Command "reboot" not found!');
            resolve();
          } else if (e === 0) {
            logs.info(host + ' is going to reboot!');
            resolve();
          } else {
            reject(e);
          }
        }
        reboot.on('error', function(e) {
          rebootHandler(e);
        });
        reboot.on('exit', function(e) {
          rebootHandler(e);
        });
              ...
              .catch(function(err) {
                ...
                if (err.message.search('No such file or directory') !== -1) {
                  ...
                  if (opts.force !== false) {
                    ...
                    return controller.closeTesselConnections([tessel]).then(reject.bind(this, err));
                  }
                } else {
        ...
        return versionFromSHA.then(function(currentVersionInfo) {
          ...
          if (version && build) {
            ...
            if (!opts.force && currentVersionInfo.version >= verifiedVersion) {
              ...
              var message = tessel.name + ' is already on the latest firmware version (' + currentVersionInfo.version + '). You can force an update with "tessel update --force".';
