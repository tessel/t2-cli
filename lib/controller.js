...
controller.uploadFirmware = function(opts) {
  return new Promise((resolve, reject) => {
    var list = require('usb').getDeviceList();
    var image = require('fs').readFileSync(opts['firmware-path']);
    var found = false;

    for (var i = 0; i < list.length; i++) {
      var device = list[i];
      // Make sure this is a Tessel
      console.log(`USB device with VID ${device.deviceDescriptor.idVendor} and PID ${device.deviceDescriptor.idProduct}`);
      if ((device.deviceDescriptor.idVendor === 0x1209) && (device.deviceDescriptor.idProduct === 0x7551)) {

        // Make sure it's in bootloader mode
        console.log('Found Tessel with bcdDevice:', device.deviceDescriptor.bcdDevice);
        if (device.deviceDescriptor.bcdDevice >> 8 === 0) {
          var DFU = require('./dfu');
          var dfu = new DFU(device, 0);
          found = true;

          logs.info('Found device in bootloader mode. Updating firmware now...');
    ...
    if (!found) {
      return reject(new Error(`No Tessels in bootloader mode found.
Ensure you are firmly holding down the button when applying power.
You should see a blinking LED (not 'breathing').`));
