...
controller.update = function(opts) {
  return new Promise(function(resolve, reject) {
    ...
    if (opts.list) {
      ...
      logs.info('Fetching available builds...');
      return updates.fetchBuildList()
        .then(function(builds) {
          logs.info('Latest builds:');
          var maxBuildsListed = 10;
          var SHAs = Object.keys(builds);

          // Limited the number of builds we print out
          SHAs = SHAs.splice(0, SHAs.length > maxBuildsListed ? maxBuildsListed : SHAs.length);
          // Print out details for each build
          for (var i = 0; i < SHAs.length; i++) {
            // Grab the specific build
            var build = builds[SHAs[i]];
            // Format the release time as a date
            var released = new Date(build.released);
            // If this is the top of the pile, it's most recent
            if (i === 0) {
              // Print when it was published
              logs.basic('\t Version:', build.version, '\tPublished:', released.toLocaleString());
            } else {
              // For the rest just print the version
              logs.basic('\t Version:', build.version);
            }
          }
          // Finish up
          return resolve();
      ...
      if (opts.build) {
        ...
        requestedVersion = opts.build;
      }

      // Grab our Tessel
      var tesselPromise = Tessel.get(opts);
      var shaPromise;

      // If we aren't forcing, we'll want to get the current SHA on Tessel 
      if (!opts.force) {
        // Once we have the Tessel
        shaPromise = tesselPromise.then(function(tessel) {
          // Figure out what commit SHA is running on it
          return tessel.fetchCurrentBuildInfo()
            // Once we have the current SHA
            .then(function(currentSHA) {
              // Figure out the human readable version for that code release
              return updates.fetchBuildVersionforSHA(currentSHA);
      ...
      // Otherwise, just resolve
        ...
        shaPromise = Promise.resolve();
      }

      // Once we have our Tessel and build version
      return Promise.all([tesselPromise, shaPromise]).spread(function(tessel, currentVersion) {
        ...
        if (opts.build) {
          ...
          return controller.updateTesselWithVersion(opts.force, tessel, currentVersion, requestedVersion)
            .then(resolve, reject);
        } else {
          // If they have requested the latest firmware
          var verifiedVersion;
          if (requestedVersion === 'latest') {
            // Figure out what the version is
            verifiedVersion = updates.fetchBuildList()
              .then(function(builds) {
                // Make sure we actually have builds
                if (Object.keys(builds).length === 0) {
                  return reject('Could not detect any builds on server.');
                }
                // Sort the builds by version
                var buildArray = updates.buildDictToSortedArray(builds);

                // Return the newest one
                return buildArray[0].version;
              });
          }
          // If they requested a specific version
          else {
            // Ensure that it exists by fetching the builds
            verifiedVersion = updates.fetchSHAForBuildVersion(requestedVersion)
              // If that function resolves, the sha exists so the version exists
              .then(function() {
          ...
          // Once we have our verified version
          verifiedVersion.then(function(requestedVersion) {
            // Check if the current build is the same or newer if this isn't a forced update
            if (!opts.force && currentVersion >= requestedVersion) {
              // If it's not, close the Tessel connection and print the error message
              var errMessage = tessel.name + ' is already on the latest firmware build (' + currentVersion + '). You can force an update with “tessel update --force”.';
              return controller.closeTesselConnections([tessel])
                .then(reject.bind(this, errMessage), reject.bind(this, errMessage));
            } else {
              if (!opts.force) {
                // If it is a newer version, let's update...
                logs.info('New firmware version found...' + requestedVersion);
              }
...
controller.updateTesselWithVersion = function(force, tessel, currentVersion, requestedVersion) {
  // Fetch the requested build
  return updates.fetchBuild(requestedVersion)
    .then(function startUpdate(build) {
      // Update Tessel with it
      return tessel.update(build)
        // Log that the update completed
        .then(function logCompletion() {
          if (!force) {
            logs.info('Updated', tessel.name, 'from ', currentVersion, ' to ', requestedVersion);
          } else {
            logs.info('Force updated', tessel.name, 'to version', requestedVersion);
          }
        })
        .then(function() {
