...
controller.doesNotMeetUSBRequirement = function(options) {
  options = options || {};
  return (typeof options.usb !== 'undefined' && options.usb === false) ||
    (typeof options.usb === 'undefined' && options.lan === true);
};

controller.provisionTessel = function(opts) {

  if (controller.doesNotMeetUSBRequirement(opts)) {
    return Promise.reject('t2 provision requires a usb connection.');
  }
...
controller.restoreTessel = function(opts) {
  if (controller.doesNotMeetUSBRequirement(opts)) {
    return Promise.reject('t2 restore requires a usb connection.');
  }
