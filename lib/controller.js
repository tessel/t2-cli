  ...
  return Promise.settle(connectionTypes)
    .reduce(function(connections, result) {
      ...
      if (result.isFulfilled()) {
        ...
        connections = connections.concat(result.value());
      }
      return connections;
    }, [])
    .then(function(connections) {
    ...
    .reduce(function(collector, connection) {
      ...
      if (match && collector.selected) {
        logs.warn('Multiple matches found. Using tessel at IP address: ' + connection.ip);
        return collector;
      }
    ...
    .then(function(connections) {
      return new Promise(function(resolve, reject) {
        ...
        if (connection.connectionType === 'LAN' && !connection.authorized) {
          ...
          return reject('Not authorized to deploy to this device. If you are connecting to Tessel 2 hardware, physically connect via USB and run `tessel provision`. If you are connecting to a VM, run `tessel key generate` prior to launching the VM.');
        }
