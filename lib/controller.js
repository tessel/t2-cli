    ...
    if (opts.list) {
      ...
      return Promise.all([tesselPromise, shaPromise]).spread(function(tessel, currentVersion) {
        ...
        if (!tessel.lanConnection || tessel.lanConnection.authorized) {
          ...
          logs.warn('Warning: LAN connection not detected for this Tessel. For a faster update, cancel and connect Tessel to LAN with the `tessel wifi` command before update.');
        }

        // If the update is forced or this build was requested
        if (opts.build) {
          // Fetch the requested build
          return controller.updateTesselWithVersion(tessel, currentVersion, requestedVersion)
            .then(resolve, reject);
        } else {
...
controller.updateTesselWithVersion = function(tessel, currentVersion, requestedVersion) {
  // Fetch the requested build
