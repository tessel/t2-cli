    ...
    Tessel.seekTessels(opts)
     ...
     if(tessels.length >= 2){
      ...
      if(tessels[0].connection.authorized){
        ...
        reject('Sorry, you are not authorized!\n\n"t2 key generate" might help :-)');
      }
      
    } else {
      reject('Sorry no nearby Tessels found!');
      process.exit();
    }
    function runSSH(id){
         // clear console
        ...
        ch.on('exit',function(e){
          if(e === 0){
            // clear console
            process.stdout.write("\u001b[2J\u001b[0;0H");
            resolve('Connection to '+tessels[id].connections[0].host+' closed!\n');
          }
          if(e === 255){
            reject('Connection to '+tessels[id].connections[0].host+' closed!\n');
          }
      ...
      function makeMenu(tessel, index){
