...
const hasOtherConnections = (kind, entries, exclusions) => {
  return entries.filter(({tessel}) => !exclusions.includes(tessel))
...
controller.runHeuristics = ({name}, tessels) => {
  ...
  return Promise.all(
    tessels.map(tessel => {
      ...
      if (!priority &&
        /* istanbul ignore next */
        (process.env.TESSEL && process.env.TESSEL === tessel.name)) {
    ...
    for (const {tessel, priority} of entries) {
      ...
      // intended Tessel.
      if (priority === NAME_OPTION_PRIORITY ||
        priority === ENV_OPTION_PRIORITY) {
        return tessel;
      }
      ...
      if (priority === USB_CONN_PRIORITY) {
        if (hasOtherConnections(USB_CONN_PRIORITY, entries, [tessel])) {
          ...
          return Promise.reject(new controller.HeuristicAmbiguityError(USB_CONN_PRIORITY));
        }
      ...
      if (priority === LAN_CONN_PRIORITY) {
        if (hasOtherConnections(LAN_CONN_PRIORITY, entries, [tessel])) {
          ...
          // the user intends to target with the given command, so we
          // exit with a useful error.
          return Promise.reject(new controller.HeuristicAmbiguityError(LAN_CONN_PRIORITY));
        }
...
controller.printAvailableNetworks = function(options) {
  ...
  return controller.standardTesselCommand(options, (tessel) => {
    ...
    return tessel.findAvailableNetworks()
      .then((networks) => {
        log.info(`Found ${networks.length} network${networks.length !== 1 ? 's' : ''} visible to ${tessel.displayName}:`);
...
controller.setWiFiState = function(options) {
  options.authorized = true;
  return controller.standardTesselCommand(options, (tessel) => tessel.setWiFiState({
    enable: options.on
  }));
};
    ...
    return tessel.getAccessPointInfo()
      .then((ap) => {
        if (ap.ssid) {
          ...
          log.info(`${tessel.displayName} is not configured as an access point (run "t2 ap --help" to learn more)`);
        }
      });
  });
};
...
controller.printAvailableUpdates = function() {
  return updates.requestBuildList().then((builds) => {
    ...
    builds.reverse().slice(0, 10).forEach((build) => {
      ...
      log.basic(`Version: ${build.version}\nPublished: ${published}\n${build.sha}\n`);
    });
  });
};
            ...
            return tessel.fetchCurrentBuildInfo()
              ...
              .catch(error => {
                ...
                if (error.message.includes('No such file or directory')) {
                  log.warn(`Could not find firmware version on ${tessel.displayName}`);
        ...
        return versionFromSHA.then(function(currentVersionInfo) {
          ...
          if (version && build) {
            ...
            if (!opts.force && semver.gte(currentVersionInfo.version, verifiedVersion)) {
              ...
              log.info(`Updating ${tessel.displayName} to latest version (${verifiedVersion})...`);
  ...
  return updates.fetchBuild(build)
    .then(image => {
      ...
      return tessel.update(opts, image)
        ...
        .then(() => {
          if (!opts.force) {
            log.info(`Updated ${tessel.displayName} from ${currentVersion} to ${build.version}`);
          } else {
            log.info(`Force updated ${tessel.displayName} to version ${build.version}`);
          }
