...
controller.deploy = function(opts) {
  ...
  return controller.standardTesselCommand(opts, function(tessel) {
    ...
    return tessel.deploy(opts).then(log.spinner.stop);
  });
};
...
controller.restart = function(opts) {
  ...
  return controller.standardTesselCommand(opts, function(tessel) {
    ...
    return tessel.restart(opts).then(log.spinner.stop);
  });
};
...
controller.eraseScript = function(opts) {
  ...
  return controller.standardTesselCommand(opts, function(tessel) {
    ...
    return tessel.eraseScript(opts, false).then(log.spinner.stop);
  });
};
  ...
  return new Promise(function(resolve, reject) {
    ...
    .then(function executeRename() {
      return controller.standardTesselCommand(opts, function(tessel) {
        ...
        log.spinner.start();
        return tessel.rename(opts).then(log.spinner.stop);
      });
    });
};
...
controller.connectToNetwork = function(opts) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    .then(() => {
      return controller.standardTesselCommand(opts, (tessel) => {
        return tessel.connectToNetwork(opts).then(log.spinner.stop);
      });
    });
};
...
controller.setWiFiState = function(opts) {
  ...
  log.spinner.start();
  return controller.standardTesselCommand(opts, (tessel) => {
    return tessel.setWiFiState(opts.on).then(log.spinner.stop);
  });
};
...
controller.createAccessPoint = function(opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then(() => {
      return controller.standardTesselCommand(opts, (tessel) => {
        return tessel.createAccessPoint(opts).then(log.spinner.stop);
      });
    });
};
        ...
        return updates.requestBuildList().then(builds => {
          ...
          return tessel.fetchCurrentNodeVersion()
            .then(nodeVersion => {
              ...
              log.info(output('Node', nodeVersion));
            })
            .catch(() => {
              log.info(output('CLI', cliVersion));
              log.info(output('Firmware', firmwareVersion));
              log.info(output('Node', 'unknown'));
            });
        });
      })
      .catch(() => {
        log.info(output('CLI', cliVersion));
        log.info(output('Firmware', 'unknown'));
        log.info(output('Node', 'unknown'));
      });
  });
};
