...
controller.reboot = function(opts) {
  return new Promise(function(resolve, reject) {
    ...
    .then(function(tessel){
      var command = 'ssh root@'+tessel.lanConnection.ip+' -i '+opts.key+' \'reboot ';
      if (opts.time || opts.delay) {
        command = command+' -d '+opts.time || opts.delay;
      }
      if(opts.force) {
        command = command+' -f ';
      }
      ...
      command = command+'\'';
      debug(command);
      var reboot_cp = cp.exec(command);

      reboot_cp.on('error', function(e){
        if(e === 255){
      ...
      reboot_cp.on('exit', function(e){
        if(e === 255){
          ...
          logs.info(tessel.name +' is going to reboot!');
          resolve();
        } else {
          reject(e);  
        }
