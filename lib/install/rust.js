...
module.exports.rustVersion = () => {
  var rustcOut = spawnSync('rustc', ['-V']);
  var version;
  try {
    version = rustcOut.stdout.toString().match(/^rustc\s+(\S+)/)[1];
  } catch (e) {}

  if (rustcOut.status !== 0 || !version) {
    throw new Error('Could not find a locally installed version of `rustc`.');
  } else {
    var cargoOut = spawnSync('cargo', ['-V']);
    if (cargoOut.status !== 0) {
      throw new Error('Could not find a locally installed version of `cargo`');
    } else {
      return version;
    }
  }
...
// This function checks the metadata of a Cargo.toml. It must be synchronous
// because checkConfiguration for a language target is synchronous.
module.exports.cargoMetadata = (destdir) => {
  var out = spawnSync('cargo', ['metadata', '--no-deps'], {
    stdio: ['ignore', 'pipe', 'inherit'],
    cwd: destdir || process.cwd(),
  });

  if (out.status !== 0) {
    throw new Error(`cargo exited with error code ${out.status}`);
  } else {
    ...
    .then(() => {
      return exports.installRustlib()
        .then(() => {
          ...
          log.error(e.message);
          log.warn('Please switch to using a stable Rust version >= 1.11.0.');
          log.warn('SDK toolchain is installed, but a libstd for your Rust version is not.');
        });
    });
};

module.exports.uninstallSdk = () => {
  return new Promise((resolve) => {
...
module.exports.checkSdk = () => {
  return exports.getBuildConfig()
    .catch(e => {
      ...
      log.error(e.message);
      log.error('Please run "cargo tessel sdk install" and try again.');
      log.error('To instead use the remote Rust compiler, use `t2 run <target> --rustcc`.');
      process.exit(1);
    });
};
...
module.exports.checkRust = (isCli) => {
  try {
    ...
    log.error('To cross-compile Tessel code on your machine, you will need both');
    log.error('`rustc` and `cargo` available in your terminal: https://rustup.rs/');
    if (isCli) {
      log.error('To instead use the remote Rust compiler, use `t2 run <target> --rustcc`.');
    }
    process.exit(1);
...
module.exports.checkConfiguration = (matchBin) => {
  var metadata = exports.cargoMetadata();
  ...
  if (validbins.length === 0) {
    ...
    if (bins.length > 0) {
      log.error('Available targets:');
      bins.forEach(bin => {
        log.error(' $ cargo tessel build --bin', bin.name);
      });
    }
    process.exit(1);
  }
...
module.exports.runBuild = (matchBin) => {
  return new Promise((resolve) => {
      exports.checkRust(true);
      resolve(exports.checkConfiguration(matchBin));
    })
    .then(out => {
