var protocol = require('./usb_daemon_protocol')
  ...
  */
  ...
  this.openProcess = function(connection, callback) {
    ...
    // If it does exist
      ...
      // Open up a child process with the given id on the remote Tessel
      connection.write(protocol.newProcess(newProc.id));
    ...
    entry.parser.on('EXIT-STATUS', function(packet) {
      ...
      if (proc) {
        ...
        proc.once('close', function() {
          ...
          delete entry.processes[packet.pid];
        });
      }
    });
        // When we get a command that a remote process has exited
