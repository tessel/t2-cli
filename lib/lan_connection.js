...
function LANConnection(opts) {
  ...
  this.auth.password = opts.password || '';
...
function findConnections(timeout) {
  ...
  return findmDNSConnections(timeout)
    .then(function(tessels){
      return new Promise(function(resolve, reject) {
          findSpecifiedRemotes(function(err, remotes) {
            if (err) {
              return reject(err);
            }
            tessels = tessels.concat(remotes);
...
// Used to gather unlisted remote Tessels (like VMs)
function findSpecifiedRemotes(callback) {
  // Read the standard file we'll store this configuration within
  fs.readFile(osenv.home() + '/.tessel/remote_config.json', function (err, buf) {
    ...
    if (err) {
      ...
      try {
        ...
        // Parse the JSON out of that file
        var remotes = JSON.parse(buf);

        // For each remote
        remotes.forEach(function(remote) {
          // Read the private key buffer into a var
          remote.privateKey = fs.readFileSync(remote.keypath);
        });

        // Return all of the remote Tessels
        return callback && callback(null, remotes);
      }
      // If there was an error parsing JSON
      catch (e) {
        // report it
