...
LAN.Connection.prototype.open = function() {
  ...
  return new Promise(function(resolve) {
    ...
    self.ssh.on('close', function(hadError) {
      // due to a closed connection is no more authorized
      self.authorized = false;
      if (hadError) {
        debug('No response from server', self.host);
        logs.warn('Tessel seams to stall ssh connection!');
        resolve(self);
      } else {
        debug('SSH connection to Tessel ' + self.host + ' closed!');
      }
      resolve(self);
    }).connect(self.auth); // trying to reconnect
    self.ssh.on('end', function() {
      debug('SSH connection to Tessel ' + self.host + ' ending!');
      // due to an ended connection is no more authorized
      self.authorized = false;
      resolve(self);
    });
  });
