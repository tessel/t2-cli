    ...
    var opening = tessel.connection.open()
      .then(() => {
        if (opts.authorized !== undefined) {
          if (tessel.connection.authorized !== opts.authorized) {
            debug(`Kicking ${conn.host.slice(0, conn.host.search(LOCAL_REGEX))} due to authorized filter: ${opts.authorized}`);
      ...
      .then(() => {
        ...
        if (conn.connectionType === 'LAN' && !conn.authorized) {
          // depending on the platform the returned host name could be "tessel-name.local." or "tessel-name.local"
          tessel.name = conn.host.slice(0, conn.host.search(LOCAL_REGEX));
