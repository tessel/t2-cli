...
TesselSeeker.prototype.stop = function() {
  ...
  if (this.usbScan !== undefined) {
    if (this.usbScanCount != this.usbScan.count) {
      logs.warn('%d Tessel device%s connected by USB may still be booting.',
        this.usbScan.count - this.usbScanCount,
        (this.usbScan.count - this.usbScanCount) == 1 ? '' : 's'); 
    }
