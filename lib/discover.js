...
TesselSeeker.prototype.start = function(onlySerialNumbers) {
  var self = this;
  ...
  function connectionHandler(conn) {
    ...
    if (onlySerialNumbers) {
      // do not open connection
      if (conn.connectionType != 'LAN') {
        conn.device.open();
        conn.device.getStringDescriptor(conn.device.deviceDescriptor.iSerialNumber, function(error, data) {
          tessel.connection.serialNumber = data;
          if (self.usbDeviceList.indexOf(tessel) == -1){
