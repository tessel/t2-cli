  ...
  setWifiStateTruthy: function(test) {
    ...
    this.tessel.setWiFiState(state)
      .then(() => {
        ...
        test.deepEqual(this.simpleExec.args[0][0], commands.turnOnWifi(state));
        test.deepEqual(this.simpleExec.args[1][0], commands.commitWirelessCredentials());
        test.deepEqual(this.simpleExec.args[2][0], commands.reconnectWifi());
        test.equal(this.logsInfo.calledOnce, true);
        test.equal(this.logsInfo.args[0][1].indexOf('Enabled.') !== -1, true);
        test.ok(this.ubusListen.callCount, 1);
        test.done();
      })
      .catch(function(err) {
  ...
  setWifiStateFalsy: function(test) {
    ...
    this.tessel.setWiFiState(state)
      .then(() => {
        ...
        test.deepEqual(this.simpleExec.args[2][0], commands.reconnectWifi());
        test.equal(this.logsInfo.calledOnce, true);
        test.equal(this.logsInfo.args[0][1].indexOf('Disabled.') !== -1, true);
        test.ok(this.ubusListen.callCount, 1);
        test.done();
      })
      .catch(function(err) {
